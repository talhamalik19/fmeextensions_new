...
const blogQuery = (page, filter) => {
    return (`
    blog(filter: {search_term: "${filter}"}, pageSize: 10, currentPage: ${page}) {
      blogs_media_url
      blogs_count
      blogs_data {
        categories {
...
export default function Blogs({ globalMagento, user, setUser, blogsCatgData, selectedCurrency, selectedStore, currencyData, storeData, slug, footer, blogBasicInfo, selectedPage, q, sarabun }) {
    ...
    const fetchBlogs = async () => {
        ...
        try {
            const response = await fetch(getStrapiURL("/graphql"), {
                ...
                body: JSON.stringify({
                    query: `{${blogQuery(page, q)}}`, variable: {},
                }),
            });
                ...
                <div className="blog_head">
                    <div className="page_info">
                        ...
                        <ul className="bdcrum">
                            ...
                            <li className="bdcrum_item"><Link href="/blog">Search Results for '{q}'</Link></li>
                        </ul>
                    </div>
                    <h2 className={`${sarabun} primary_title`}>{dlv(blogBasicInfo, '0.title')}</h2>
                </div>
            </div>
            <CategoryBlog selectedStore={selectedStore} blogBasicInfo={blogBasicInfo} categories={[]} blogs={blogs} blogsCatgData={[]} handleCategoryClick={handleCategoryClick} blogs_media_url={blogs_media_url} totalBlogsCount={totalBlogsCount} page={page} onPageChange={onPageChange} sarabun={sarabun}/>
        </Layout>
    )
}
...
export async function getServerSideProps(context) {
    ...
    try {
        ...
        return {
            props: { blogsCatgData: categoriesBlogs || null, preview: context.preview || null, slug: [context.resolvedUrl.replace(/\/\//g, '/')], blogBasicInfo: blog || null, globalMagento: globalMagento, storeData: storeCurrency, currencyData: storeCurrency.data.currency, selectedPage: page, q: q },
        };
    } catch (error) {
        return {
