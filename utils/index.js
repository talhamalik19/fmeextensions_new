...
export function getData(slug, locale, apiID, kind, preview) {
  ...
  if (kind == 'collectionType') {
    ...
    const apiUrl = `/${pluralize(
      apiID
    )}?filters[slug][$eq]=${slug}&locale=${locale}${previewParams}&populate[blocks][populate]=members.picture,header,buttons.link, buttons.button.link,faq,featuresCheck,cards,pricingCards.perks,articles,restaurants,author.picture,images,cards.image,image&populate=localizations&populate[seo][populate]=metaSocial.image`;
    ...
    const apiUrl = `/${apiID}?locale=${locale}${previewParams}&populate[blocks][populate]=*,buttons.link,buttons.button.link&populate=localizations&populate[header]=*&populate[seo]=metaSocial`;
